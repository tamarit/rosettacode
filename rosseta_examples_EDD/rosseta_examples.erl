...
calculate_one(Title0) ->
   ...
   case httpc:request(URL) of
      {ok,Result} ->
            % {match,Found} = 
            %    re:run(lists:last(tuple_to_list(Result)), "\n=={{header(|Erlang)", [global]),
            % io:format("~p\n",[Found]),
            % io:format("~s\n",[lists:last(tuple_to_list(Result))]),
            Found = search_erlang(lists:last(tuple_to_list(Result))),
            io:format("{\"~ts\", ~p},\n",[Title0,Found]),
            Found;
...
search_erlang(List = [_|Tail]) ->
   Searched = "=={{header|Erlang}}==",
   case lists:prefix(Searched, List) of 
      true -> 
         search_for_another(0,lists:sublist(List, length(Searched), length(List)));
      false ->
         search_erlang(Tail)
   end;
search_erlang([]) ->
...
search_for_another(N,List = [_|Tail]) ->
   Searched = "=={{header(|",
   case lists:prefix(Searched, List) of 
      true -> 
         N;
      false ->
         search_for_another(N+1,Tail)
   end;
search_for_another(N, []) ->
   N.
